...
Source code for the Decentralized LLM Standard subnet dashboard: https://dash.hypertensor.org
```
Swarm state: healthy
...
See more info about DLMS (Decentralized LLM Standard) in its [main GitHub repo](https://github.com/hypertensor-blockchain).
...
```bash
git clone https://github.com/hypertensor-blockchain/subnet-dash
cd subnet-dash
python -m venv .venv
...
```bash
git https://github.com/hypertensor-blockchain/subnet-dash
cd subnet-dash
...
To monitor your private swarm instead of the public one, please replace `PUBLIC_INITIAL_PEERS` with a list of multiaddresses of your swarm's **initial peers** in [config.py](config.py). 
...
- **GET /api/v1/state** ([example](https://dash.hypertensor.org/api/v1/state))

    This call returns a large JSON that contains all data used for rendering the [health monitor page](https://dash.hypertensor.org/).
...
- **GET /api/v1/is_reachable/`<peer_id>`**
    ...
    Subnet servers use this call to ping themselves and ensure that they are reachable before announcing their ONLINE status to the swarm.
